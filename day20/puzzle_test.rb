...
RSpec.describe Puzzle do
  ...
  describe "#corners" do
    it "returns all corner pieces" do
      expect(@puzzle.corners.map{|t|t.id}).to eql(["1951", "1171", "2971", "3079"])
    end
  end

  describe "#neighbors" do
    it "builds an adjacency list of tiles" do
      expect(@puzzle.neighbors[@t2311].map{|t|t.id}).to eql(["1951", "1427", "3079"])
    end
  end

  describe "#neighbors_of" do
    it "returns an array of neighboring tiles" do
      expect(@puzzle.neighbors_of(@t2311).map{|t|t.id}).to eql(["1951", "1427", "3079"])
    end
  end
